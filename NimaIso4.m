...
for colorIdx=1:4  % loop through colors
    ...
    % set defailt values
    rollInfo.mostRightX(colorIdx)=13;
    ...
    if(~gameInfo.closedColors(colorIdx))
        ...
        if(rollInfo.xNum(colorIdx)>0)
            rollInfo.mostRightX(colorIdx) = find(colorRow == 1,1,'last');
        else
            rollInfo.mostRightX(colorIdx) = 0;
        end
        ...
        if(rollInfo.mostRightX(colorIdx) < indexOfValue)
            rollInfo.Wpos(colorIdx) = indexOfValue;
            rollInfo.WskipProb(colorIdx) = sum(probVec(rollInfo.mostRightX(colorIdx)+1:indexOfValue-1));
        ...
        if (isItMyTurn)
            ...
            if (rollInfo.mostRightX(colorIdx) >= indexOfValue(2)) % both options don't work
                indexOfValue=[0,0];
            elseif (rollInfo.mostRightX(colorIdx) >= indexOfValue(1)) % option 2 works but one does not
                indexOfValue(1) = 0;
            ...
            for ii=1:2
                if(indexOfValue(ii)>0)
                    ...
                    rollInfo.CskipProb(colorIdx,ii) = sum(probVec(rollInfo.mostRightX(colorIdx)+1:indexOfValue(ii)-1));
