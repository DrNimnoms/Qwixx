function [ color , number ] = Tatyana( gameInfo, playerID)
...
if (debug == 1)
 str = sprintf('\n\nTatyana: Action[%d]', gameInfo.action);
 disp(str);
end
...
colorIndexOfDiceSum = findRowIndicies (gameInfo);
...
function  colorIndexOfDiceSum = findRowIndicies (gameInfo)
for i = 1:4
    list = getRowValues(i);
    if (gameInfo.action == 1)
        sumOfTheTwoDice = gameInfo.dice.white(2) + gameInfo.dice.white(1);
    elseif(gameInfo.action == 2)
        valueOfColorDie = gameInfo.dice.(cell2mat(gameInfo.colorOptions(i)));
        Combo1 = valueOfColorDie + gameInfo.dice.white(1);
        Combo2 = valueOfColorDie + gameInfo.dice.white(2);
        if ( find(list == Combo1) <= find(list == Combo2) )
            sumOfTheTwoDice = Combo1;
        else
            sumOfTheTwoDice = Combo2;
        end
    end
    colorIndexOfDiceSum(i) = find(list == sumOfTheTwoDice);
end
end
